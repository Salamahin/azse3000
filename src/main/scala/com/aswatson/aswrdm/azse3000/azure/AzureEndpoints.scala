...
//
//import com.aswatson.aswrdm.azse3000.shared.{Account, Container, Path, Secret}
...
//
//  def unapply(uri: URI): Option[(Account, Container, Path)] = uri.toString match {
//    case uriTemplate(acc, cont, path) => Some(Account(acc), Container(cont), Path(URLDecoder.decode(path, "UTF-8")))
//    case _                            => None
//  }
//
//  def toContainer(account: Account, container: Container, token: Secret) = new CloudBlobContainer(
