...
final case class ParseCommand(cmd: Command) extends Parse2[Either[MalformedCommand, Expression[ParsedPath]]]
...
final case class StartListing(inPath: ParsedPath, secret: Secret)
    extends Azure[Either[ContainerListingFailed, ResultSegment[ListBlobItem]]]
...
final case class StartContentCopying(from: ParsedPath, fromBlob: CloudBlockBlob, to: ParsedPath, toSecret: Secret)
    extends Azure[Either[BlobCreationFailed, CloudBlockBlob]]
...
final case class UserInterface[F[_]]()(implicit inj: InjectK[UI, F]) {
  ...
  def showProgress(op: Description, progress: Int) = ExecStrategy(inj(ShowProgress(op, progress)))
}
